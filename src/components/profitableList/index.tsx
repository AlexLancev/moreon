...
import { profitableVisitsStore } from "@/stores/dataStore";
import { useGetResponsiveValue } from "@/utils";

import { ContentLoader } from "../contentLoader";

export const ProfitableList = observer(() => {
  const { data } = profitableVisitsStore;
  ...
  return (
    ...
    >
      <ContentLoader
        currentStore={profitableVisitsStore}
        skeletonComponent={SkeletonBeautifulFigureSection}
        initialVisibleCount={4}
      >
        {data.map(({ head, profitableVisitsList, imagesUrl }, idx: number) => {
          if (!head && !profitableVisitsList && !imagesUrl) return null;
          ...
          return (
            ...
            >
              ...
              <ul className="list-disc pb-2">
                {profitableVisitsList.map((profit, idx: number) => {
