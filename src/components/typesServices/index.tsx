...
import {
  ...
  SkeletonServiceCard,
  sizeRangesData,
  typesServicesDataDecor,
} from "@/constans";
import { typesServicesStore } from "@/stores/dataStore";
import { isEmptyObj, useGetResponsiveValue } from "@/utils";

const renderIcons = (path: TypesServicesKey): ReactNode | null => {
  if (isEmptyObj(typesServicesDataDecor)) return null;

  return typesServicesDataDecor[path];
};

export const TypesServices = observer(() => {
  const { data } = typesServicesStore;
  ...
  return (
    <div>
      ...
      >
        <ContentLoader
          currentStore={typesServicesStore}
