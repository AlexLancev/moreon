...
import { videoReviewsStore } from "@/stores/dataStore";
import { useGetResponsiveValue } from "@/utils";

import { ContentLoader } from "../contentLoader";
import { AccessibleButton } from "../ui/accessibleButton";

export const VideoReviewsList = observer(() => {
  const [isVideoActive, setIsVideoActive] = useState<null | number>(null);
  const { data } = videoReviewsStore;
  ...
  return (
    ...
    >
      <ContentLoader
        currentStore={videoReviewsStore}
        skeletonComponent={SkeletonVideoReviews}
        initialVisibleCount={4}
      >
        {data.map(
          (
            { description, personAboutClub, previewImage: { jpg, webp }, path },
            idx: number,
          ) => (
