...
export const Feedback = () => {
  if (!contacts_data || isEmptyObj(contacts_data) || !feedback_data || feedback_data.length === 0) return null;
    ...
    <ul className="flex items-center justify-end gap-x-6 text-xs text-[rgba(255,255,255,0.8)]">
      {feedback_data?.map((contact, idx: number) => (
        <li key={idx} className="flex items-center gap-x-2">
          <picture>
            <source srcSet={contacts_data[contact]?.images_url?.svg} type="image/svg+xml" />
            <img width={17} src={contacts_data[contact]?.images_url?.png} alt="" aria-hidden />
          </picture>
          <a
          ...
          >
            {contacts_data[contact]?.name === "Адрес" ? <address>{contacts_data[contact]?.text}</address> : contacts_data[contact]?.text}
          </a>
        </li>
      ))}
    </ul>
