...
export const Team_list = ({ isActiveTab }: Team_list_type) => {
  const selected_category_data = team_data?.filter(({ type }) => type?.[isActiveTab]);

  const renderSlide = (startIndex: number, endIndex: number) => (
    <SwiperSlide key={startIndex} className="min-h-[348px]">
      {selected_category_data.slice(startIndex, endIndex).map(({ name, url_images: { webp, jpg }, about_coach: { qualification } }, idx: number) => {
        return (
          <Link
            key={idx}
            to="/"
          ...
          >
            <>
              ...
              <strong className="absolute z-10 bottom-5 left-5 text-[#d6d6d6] text-lg">{name}</strong>
            </>
          </Link>
        );
      })}
  ...
  if (!selected_category_data || selected_category_data.length === 0) return null;
  ...
  return (
    <Swiper className="team_list" modules={[Pagination]} slidesPerView={4} spaceBetween={20} pagination={{ clickable: true }}>
      {slides}
    </Swiper>
  );
};
